...
class Cortex:
    ...
    def visrois_dict(self, subjects=None, verbose: bool = False):
        """
        Loads ROI masks for specified subjects (or all if None).
        Returns: rois, binary_masks, viscortex_masks (same format as before)
        """
        binary_masks = {}
        viscortex_masks = {}
        rois = None
        ...
        if subjects is None:
            subjects = self.nsp.subjects

        for subject in subjects:
            mask_dir = (
                f"{self.nsp.nsd_datapath}/nsddata/ppdata/subj0{subject[-1]}/func1mm/roi"
            )
            mask_files = sorted(
                f for f in os.listdir(mask_dir) if f.endswith("_mask.nii.gz")
            )

            subj_masks = {}
            print(f"Loading masks for subject {subject}...") if verbose else None

            for mask_file in mask_files:
                roi_name = mask_file[
                    :-12
                ]  # Remove '_mask.nii.gz', kan allemaal mooier, maar veel funcs hangen hier vanaf
                mask_data = (
                    nib.load(os.path.join(mask_dir, mask_file)).get_fdata().astype(int)
                )
                subj_masks[roi_name] = mask_data
                print(f" - {roi_name}: {np.sum(mask_data)} voxels") if verbose else None

            binary_masks[subject] = subj_masks

            if rois is None:
                rois = list(subj_masks.keys())

            viscortex_masks[subject] = np.sum(list(subj_masks.values()), axis=0)

        return rois, binary_masks, viscortex_masks

    def prf_dict(self, subjects: str | None, rois: list, roi_masks: dict):
        """
        Efficiently builds a dictionary with all the pRF data for all subjects and ROIs.
        Output format is identical to the original function.
        """
        ...
        roi_list = [f"{roistr}" for roistr in rois]
        ...
        for subject in subjects:
            # Load all prf arrays for this subject once
            prf_arrays = {}
            for prf_type in prf_types:
                # Only keep the array (prf_ar) for efficiency
                _, prf_ar, _, _ = self.nsp.datafetch.get_dat(
                    f"{self.nsp.nsd_datapath}/nsddata/ppdata/{subject}/func1mm/prf_{prf_type}.nii.gz"
                )
                prf_arrays[prf_type] = prf_ar

            # Build nsd_dat dict (format preserved)
            nsd_dat = {
                prf_type: {"prf_ar": prf_arrays[prf_type]} for prf_type in prf_types
            }

            # Build proc dict (format preserved)
            proc = {}
            # for f"{roi}_mask" in roi_list:
            for roi in roi_list:

                roi_mask = roi_masks[subject][roi]
                proc[f"{roi}_mask"] = {
                    prf_type: self.nsp.utils.roi_filter(roi_mask, prf_arrays[prf_type])
                    for prf_type in prf_types
                }
                # Calculate lin_sigma as before
                size = proc[f"{roi}_mask"]["size"]
                exponent = proc[f"{roi}_mask"]["exponent"]
                lin_sigmas = size[:, 3] * np.sqrt(exponent[:, 3])
                proc[f"{roi}_mask"]["lin_sigma"] = np.column_stack(
                    [size[:, 0:3], lin_sigmas]
                )

            prf_dict[subject] = {"nsd_dat": nsd_dat, "proc": proc}

        return prf_dict

    def calculate_pRF_location(
        self,
        prf_size: np.ndarray,
        prf_ecc: np.ndarray,
        prf_angle: np.ndarray,
        image_size: Tuple[int, int] = (425, 425),
        visual_angle_extent: float = 8.4,
        ...
        r_index = (image_size[0] + 1) / 2 + (
            prf_ecc
            * np.sin(np.radians(prf_angle))
            * (image_size[0] / visual_angle_extent)
        )

        # Calculate pRF x-position in column pixel units
        c_index = (image_size[1] + 1) / 2 + (
            prf_ecc
            * np.cos(np.radians(prf_angle))
            * (image_size[0] / visual_angle_extent)
        )
    ...
    def anat_templates(self, prf_dict: Dict) -> Dict[str, nib.Nifti1Image]:
        ...
        Returns:
        dict: A dictionary where each key is a subject ID and each value is a Nifti1Image object representing the subject's T1w anatomical map.
        """
        anat_temps = {}
        for subject in prf_dict.keys():
            anat_temps[subject] = nib.load(
                f"{self.nsp.nsd_datapath}/nsddata/ppdata/{subject}/func1mm/T1_to_func1mm.nii.gz"
            )
        return anat_temps
    ...
    def nsd_R2_dict(
        self, subjects: list | None, roi_masks: dict, glm_type: str = "hrf"
    ):
        """
        ...
        # Use all subjects if none specified # This I have changed, it used to be all subs automatically
        if subjects is None:
            subjects = self.nsp.subjects
        ...
        for subject in subjects:
            ...
            roi_list = [f"V{i + 1}" for i in range(len(roi_masks[subject]))]
            if glm_type == "onoff":
            ...
            nsd_R2_dat, nsd_R2_ar, nsd_R2_dim, nsd_R2_range = (
                self.nsp.datafetch.get_dat(nsd_R2_path)
            )
            nsd_R2_dict[subject]["full_R2"] = {
                "R2_dat": nsd_R2_dat,
                "R2_ar": nsd_R2_ar,
            ...
            for roi in roi_list:
                nsd_R2_dict[subject]["R2_roi"][roi] = self.nsp.utils.roi_filter(
                    roi_masks[subject][roi], nsd_R2_dict[subject]["full_R2"]["R2_ar"]
                )
    ...
    # This is essentially the same as what's present inside the get_mask function, but now it just gives you a list of voxels
    def find_top_voxels(
        self,
        subject="subj01",
        roi="V1",
        ...
        roi_voxels = self.nsp.utils.numpy2coords(
            vismask_dict[subject][f"{roi}_mask"], keep_vals=False
        )
        ...
        for n_vox, these_coords in enumerate(roi_voxels):
            ...
            for prf_par in prf_pars:
                prf_par_vals = prf_dict[subject]["nsd_dat"][prf_par]["prf_ar"][
                    these_coords[0], these_coords[1], these_coords[2]
                ]
                voxel_pars[prf_par] = prf_par_vals
            # Also add the NSD Rsquared value to the dict.
            voxel_pars["nsdR2"] = R2_dict[subject]["full_R2"]["R2_ar"][
                these_coords[0], these_coords[1], these_coords[2]
            ]
            ...
            if (
                min_ecc
                <= voxel_pars["eccentricity"] + (voxel_pars["size"] / 2)
                <= max_ecc
                and min_size <= voxel_pars["size"] <= max_size
                and min_prf_R2 <= voxel_pars["R2"]
                and min_hrf_R2 <= voxel_pars["nsdR2"]
            ):
    ...
    def get_good_voxel(
        ...
        if pick_manually is not None:
            ...
            which = random.randint(
                1, hrf_dict[subject][f"{roi}_mask"][select_values].shape[0]
            )
        indices = tuple(
            self.nsp.utils.sort_by_column(
                hrf_dict[subject][f"{roi}_mask"][select_values], 3, top_n=100000
            )[which, :3].astype("int")
        )

        voxelname = self.nsp.cortex.get_voxname_for_xyz(
            xyz_to_voxname, indices[0], indices[1], indices[2]
        )
    ...
    def plot_top_vox(
        ...
        y = ((1 + dim) / 2) - (
            prf_ecc * np.sin(np.radians(prf_angle)) * (dim / 8.4)
        )  # y in pix (c_index)
        x = ((1 + dim) / 2) + (
            prf_ecc * np.cos(np.radians(prf_angle)) * (dim / 8.4)
        )  # x in pix (r_index)

        if type == "circle" or type == "gaussian":
            deg_radius = sigma
            pix_radius = sigma_pure  # arbitrary
        elif type == "cut_gaussian" or type == "full_gaussian" or type == "outline":
            deg_radius = prf_size
            pix_radius = prf_size * (dim / 8.4)  # arbitrary
        ...
        if type == "gaussian":
            ...
            prf_mask = self.nsp.utils.make_gaussian_2d(
                dim, x, y, prf_size * (dim / 8.4)
            )
        elif type == "cut_gaussian":
            prf_mask = self.nsp.utils.css_gaussian_cut(
                dim, x, y, prf_size * (dim / 8.4)
            ).reshape((425, 425))
        else:
            raise ValueError(
                f"Invalid type: {type}. Available mask types are 'gaussian','circle','full_gaussian','cut_gaussian'."
            )

        central_patch = 0
        if add_central_patch:
            central_patch = self.nsp.utils.make_circle_mask(
                dim,
                ((dim + 2) / 2),
                ((dim + 2) / 2),
                outline_rad * (dim / 8.4),
        ...
        ax.imshow(
            (prf_mask + central_patch),
            cmap="bone",
            origin="upper",
            extent=[-4.2, 4.2, -4.2, 4.2],
        )
        ax.set_title(
    ...
    def rsquare_selection(
        self, input_dict=None, top_n=1000, n_subjects=None, dataset="nsd"
    ):
        rsq_dict = {}
    ...
    def optimize_rsquare(
        ...
        while True:
            ...
            highR2 = self.rsquare_selection(
                R2_dict_hrf, top_n, n_subjects=8, dataset=dataset
            )
    ...
    def get_mask(
        ...
        if isinstance(peripheral_center, tuple):
            ...
            patchloc_triangle_s = np.sqrt(
                patchloc_triangle_o**2 + patchloc_triangle_a**2
            )  # Pythagoras triangle side s, patch center eccentricity
            if peri_info:
                print(
                    f"Patch localisation triangle with side lengths o: {round(patchloc_triangle_o, 2)}, a: {round(patchloc_triangle_a,2)}, s: {round(patchloc_triangle_s,2)}"
                )
            ...
            patch_bound_angle = np.degrees(
                np.arctan(bound_triangle_o / bound_triangle_a)
            )
            patch_center_angle = np.degrees(
                np.arctan(np.abs(peripheral_center[1] / peripheral_center[0]))
            )
            ...
            if peri_info:
                ...
                print(
                    f"Peripheral patch at angle {round(patch_center_angle,2)} with boundary angles at min: {round(angle_min,2)}, max: {round(angle_max,2)}"
                )

        # Create objects for all the required pRF data
        roi_mask_data = prf_proc_dict[subject]["proc"][f"{roi}_mask"]
        angle_roi, ecc_roi, expt_roi, size_roi, rsq_roi, gain_roi, meanvol_roi = (
            roi_mask_data["angle"],
            roi_mask_data["eccentricity"],
            roi_mask_data["exponent"],
            roi_mask_data["size"],
            roi_mask_data["R2"],
            roi_mask_data["gain"],
            roi_mask_data["meanvol"],
        )
        ...
        if filter_dict != None:
            ...
            mask = np.any(
                np.all(angle_roi[:, None, :3] == smaller_xyz, axis=-1), axis=1
            )
        else:
        ...
        if heatmap == "n":
            prf_vec = random.sample(
                range(angle_roi[mask].shape[0]), angle_roi[mask].shape[0]
            )
            iter = 0
        ...
        while True:
            ...
            prf_angle, prf_ecc, prf_expt, prf_size, prf_rsq, prf_gain, prf_meanvol = (
                angle_roi[mask][n][3],
                ecc_roi[mask][n][3],
                expt_roi[mask][n][3],
                size_roi[mask][n][3],
                rsq_roi[mask][n][3],
                gain_roi[mask][n][3],
                meanvol_roi[mask][n][3],
            )
            x_vox, y_vox, z_vox = (
                int(angle_roi[mask][n][0]),
                int(angle_roi[mask][n][1]),
                int(angle_roi[mask][n][2]),
            )
            ...
            if peripheral_center == None:
                ...
                center_y = middle_xy[1] - (
                    peripheral_center[1] * dim / 8.4
                )  # This is reversed because that's how they do it
            ...
            if all(valid_conditions):
                if ecc_strict == "y" and min_overlap < 100:  # Fix this condiional
                    ...
                    region_patch = self.nsp.utils.make_circle_mask(
                        dim, center_x, center_y, patch_radius * dim / 8.4, fill="y"
                    )
                    bin_prf = self.nsp.utils.make_circle_mask(dim, x, y, pix_radius)
                    prop_in_patch = (
                        np.sum(bin_prf * region_patch) / np.sum(bin_prf)
                    ) * 100
                ...
                prop_condition = prop_in_patch >= min_overlap
                if prop_condition:
                    break
            ...
            # Check for argument option to print reason for excluding voxels
                ...
                print(
                    f"Discarding pRF mask for voxel [{x_vox}, {y_vox}, {z_vox}] due to:"
                )
                conditions_messages = [
                    ...
                    (
                        f"pRF outside of central {2 * ecc_max}° visual degrees",
                        valid_conditions[4],
                    ),
                    (
                        f"pRF does not overlap enough with central patch: {prop_in_patch}% of required {min_overlap}%",
                        valid_conditions[5],
                    ),
                    (
                        f"pRF angle not within predetermined range of {angle_min}° to {angle_max}°",
                        valid_conditions[6],
                    ),
                    (
                        f"This voxel's NSD R-squared does not explain more than {nsdR2_min}% of the fMRI signal variance",
                        valid_conditions[7],
                    ),
                    (
                        f"This voxel's pRF R-squared does not explain more than {nsdR2_min}% of the fMRI signal variance",
                        valid_conditions[7],
                    ),
                ]
        ...
        if type == "gaussian":
            ...
            prf_mask = self.nsp.utils.make_circle_mask(
                dim,
                ((dim + 2) / 2),
                ((dim + 2) / 2),
                ecc_max * (dim / 8.4),
            ...
            raise ValueError(
                f"Invalid type: {type}. Available mask types are 'gaussian','circle','full_gaussian','cut_gaussian', and 'outline'."
            )
    ...
    def prf_heatmap(
        ...
        for subject in subjects:
            ...
            if filter_dict != None:
                ...
                filter = np.any(
                    np.all(
                        prf_proc_dict[subject]["proc"][f"{roi}_mask"]["angle"][
                            :, None, :3
                        ]
                        == smaller_xyz,
                        axis=-1,
                ...
                roi_flt = filter_dict[subject][f"{roi}_mask"].shape[
                    0
                ]  # Amount of voxels in top rsq dict for subj, roi
                prf_vec = random.sample(
                    range(roi_flt), roi_flt
                )  # Create random vector to shuffle order voxels to consider

            else:
                filter = range(
                    0, prf_proc_dict[subject]["proc"][f"{roi}_mask"]["angle"].shape[0]
                )
                roi_flt = np.sum(binary_masks[subject][f"{roi}_mask"]).astype(
                    "int"
                )  # This is the total number of voxels for subj, roi
                prf_vec = random.sample(
                    range(roi_flt), roi_flt
                )  # Create random vector to shuffle order voxels to consider

            if n_prfs == "all":
                n_prfs_subject = np.sum(
                    binary_masks[subject][f"{roi}_mask"]
                )  # NOTE: consider
            else:
            ...
            prfmask_dict[subject][f"{roi}_mask"] = np.zeros(
                binary_masks[subject][f"{roi}_mask"].shape
            )
            ...
            for prf in range(n_prfs_subject):
                try:
                    ...
                    prfmask_dict[subject][f"{roi}_mask"][prf_dict["x_vox"]][
                        prf_dict["y_vox"]
                    ][prf_dict["z_vox"]] = 1

                    if print_prog == "y":
                        print(
                            f"Subject: {subject}, Voxel {prf+1} out of {n_prfs_subject} found"
                        )
                    ...
                    if prf >= n_prfs_subject:
                        print(
                            f"All potential pRFs have been considered at least once.\n"
        ...
        outline = self.nsp.utils.make_circle_mask(
            dim, center_x, center_y, outline_degs * dim / 8.4, fill=fill_outline
        )

        # Create a circle outline if an array is provide in the outline argument (should be same dimensions, binary)
        prf_sum_all_subjects += (
            (np.max(prf_sum_all_subjects) * outline) if outline_degs is not None else 1
        )
    ...
    def compare_heatmaps(
        ...
        def plot_mask(
            ax, mask, title, subtitle="aars", last=None, extent=[-4.2, 4.2, -4.2, 4.2]
        ):
        ...
        for n, roi in enumerate(rois):
            ...
            plot_mask(
                ...
                subtitle=(
                    ...
                    f"Average overlap with central patch: {round(np.mean(prf_overlaps), 2)}%"
                ),
                last=last_plot,
            )
    ...
    def compare_heatmaps_clean(
        ...
        def plot_mask(
            ax, mask, title, subtitle="", last=None, extent=[-4.2, 4.2, -4.2, 4.2]
        ):
            img = ax.imshow(mask, cmap=cmap, extent=extent)
            ax.set_title(title, fontsize=13, weight="semibold")
        ...
        heatmaps = []
        for n, roi in enumerate(rois):
            heatmap, _, _, roi, prf_sizes, rel_surf, _, prfmask_dict_all, _ = (
                self.prf_heatmap(
                    n_prfs,
                    binary_masks=prfmask_dict_all,
                    prf_proc_dict=prf_proc_dict,
    ...
    def viscortex_plot(
        ...
        distinct_roi_colours: bool = True,
        inv_colour: bool = False,
        cmap="hot",
        lowcap: Optional[float] = None,
        upcap: Optional[float] = None,
        regresult: Optional[np.ndarray] = None,
    ):
        ...
        for roi_factor, roi in enumerate(vismask_dict[subject].keys()):
            if distinct_roi_colours:
                roi_facor = 1
            mask_viscortex += self.nsp.utils.cap_values(
                vismask_dict[subject][roi],
                lower_threshold=lowcap,
                upper_threshold=upcap,
            ) * ((roi_factor + 1))
        ...
        if isinstance(plot_param, str):
            ...
            if plot_param == "nsdR2":
                ...
                brain = np.nan_to_num(
                    prf_dict[subject]["nsd_dat"][plot_param]["prf_ar"]
                )
            ...
            comrows = self.nsp.utils.find_common_rows(
                brain_flat, mask_flat, keep_vals=True
            )

            slice_flt = self.nsp.utils.cap_values(
                self.nsp.utils.coords2numpy(
                    coordinates=comrows, shape=brain.shape, keep_vals=True
                ),
                lower_threshold=lowcap,
                upper_threshold=upcap,
            )
            ...
            slice_flt = self.nsp.utils.cap_values(
                np.copy(regresult), lower_threshold=lowcap, upper_threshold=upcap
            )
        ...
        z_slider = widgets.IntSlider(
            min=0, max=slice_flt.shape[2] - 1, description="saggital"
        )
        x_slider = widgets.IntSlider(
            min=0, max=slice_flt.shape[0] - 1, description="horizontal:"
        )
        ...
        def _update_plot(x, y, z):
            ...
            im1 = axs[0].imshow(
                img1,
                cmap=f"{cmap}{rev}",
                vmin=np.min(mask_viscortex),
                vmax=np.max(mask_viscortex),
            )
            axs[0].axis("off")
            ...
            im2 = axs[1].imshow(
                img2,
                cmap=f"{cmap}{rev}",
                vmin=np.min(slice_flt),
                vmax=np.max(slice_flt),
            )
        ...
        widgets.interact(
            _update_plot, x=slice_flt.shape[0] - 1, y=y_slider, z=slice_flt.shape[2] - 1
        )

    # TODO: Add pRF, voxel information in the plot title as option.
    def plot_prfs(
        self,
        voxelsieve: VoxelSieve,
        which_voxels: Union[int, Sequence[int], str] = "all",
        cmap: str = "bone",
        enlarge: bool = True,
        sort_by: str = "random",
    ) -> None:
        ...
        n_voxels = np.sum(
            voxelsieve.vox_pick
        )  # Get the total amount of voxels, r2mask, because this is the final boolmask
        ...
        prfs = np.sum(
            self.nsp.utils.css_gaussian_cut(
                dims,
                voxelsieve.ycoor.reshape(-1, 1)[which_voxels],
                voxelsieve.xcoor.reshape(-1, 1)[which_voxels],
                voxelsieve.size.reshape(-1, 1)[which_voxels] * (425 / 8.4),
            ),
            axis=0,
        )
        print(
            f"Patch centre at y: {voxelsieve.patchcoords[1]}, and x:{voxelsieve.patchcoords[0]}"
        )
        central_patch = np.max(prfs) * self.nsp.utils.make_circle_mask(
            voxelsieve.figdims[0],
            voxelsieve.patchcoords[1],
            voxelsieve.patchcoords[0],
            voxelsieve.patchbound * (dims[0] / 8.4),
            fill="n",
        )
    ...
    def vox_per_hemi(
        ...
        for hemisphere in ["lh", "rh"]:
            hemdat = nib.load(
                f"{self.nsp.nsd_datapath}/nsddata/ppdata/{subject}/func1mm/roi/{hemisphere}.prf-visualrois.nii.gz"
            ).get_fdata()
            visroi_mask = hemdat > 0
            if all_voxels:
                for roi in rois:
                    # Store the result in the DataFrame
                    df.loc[roi, hemisphere] = roi_masks[subject][f"{roi}_mask"][
                        visroi_mask
                    ].sum()
            else:
                for roi in rois:
                    # Store the result in the DataFrame
                    df.loc[roi, hemisphere] = self.nsp.utils.coords2numpy(
                        voxeldict[roi].xyz, roi_masks[subject]["V1_mask"].shape
                    )[visroi_mask].sum()
