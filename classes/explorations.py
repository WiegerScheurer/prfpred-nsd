...
class Explorations:
    ...
    def compare_radius(
        ...
        for k, ax in enumerate(axes):
            ...
            bright_palette = sns.color_palette(
                "bright", n_colors=len(prf_dictionary["subj01"]["proc"])
            )
            ...
            for subject, dict_shelve in prf_dictionary.items():
                for j, (roi, data) in enumerate(dict_shelve["proc"].items()):
                    ...
                    if k == 0:
                        ...
                        y_data = sigma_values[
                            :, 3
                        ]  # Assuming 4th column of linear sigma
                    ...
                    all_subs["y"][f"{roi[:2]}"] = np.concatenate(
                        [all_subs["y"][f"{roi[:2]}"], y_data]
                    )
                    all_subs["x"][f"{roi[:2]}"] = np.concatenate(
                        [all_subs["x"][f"{roi[:2]}"], x_data]
                    )

            for n_col, roi in enumerate(["V1", "V2", "V3", "V4"]):
                # Collect legend handles for each ROI
                legend_handles.append(
                    Line2D([0], [0], color=bright_palette[n_col], lw=2, label=f"{roi}")
                )
            ...
            ax.legend(
                handles=legend_handles, bbox_to_anchor=(1.05, 1), loc="upper left"
            )
    ...
    def prf_plots_new(self, subject: str):
        ...
        for idx in prf_info:
            _, prf_plot_dict[idx], _, prf_range = self.nsp.datafetch.get_dat(
                f"{self.nsp.nsd_datapath}/nsddata/ppdata/{subject}/func1mm/prf_{idx}.nii.gz"
            )

        # Calculate the common boolean mask based on exclusion criteria
        common_mask = (
            (~np.isnan(prf_plot_dict["eccentricity"]))
            & (prf_plot_dict["eccentricity"] < 1000)
            & (prf_plot_dict["size"] < 1000)
        ...
        sigma_array, x, y = self.nsp.utils.calculate_sigma(
            prf_plot_dict["eccentricity"], prf_plot_dict["angle"]
        )
        ...
        (
            prf_plot_dict["sigma_px"],
            prf_plot_dict["r_index"],
            prf_plot_dict["c_index"],
        ) = self.nsp.cortex.calculate_pRF_location(
        ...
        for key, value in prf_plot_dict.items():
            ...
            plt.hist(
                np.log1p(value.flatten()), bins=num_bins, color="red", alpha=0.7
            )  # Using np.log1p to handle zero values
        ...
        plt.scatter(
            prf_plot_dict["x"],
            prf_plot_dict["y"],
            c=prf_plot_dict["size"],
            cmap="cividis",
            s=prf_plot_dict["size"],
            alpha=0.7,
        )
    ...
    def rsq_to_size(self, prf_dict: dict, roi_masks: dict, rsq_type="nsd"):
        sns.set_style("white")
        bright_palette = sns.color_palette(
            "bright", n_colors=len(prf_dict["subj01"]["proc"])
        )
        if rsq_type == "nsd":
        ...
        for i, roi in enumerate(list(prf_dict["subj01"]["proc"].keys())):
            ...
            # Plotting
            sns.scatterplot(
                x="Size", y="RSQ", data=df, color=bright_palette[i], ax=axes[i], s=1
            )
        ...
        plt.suptitle(
            "Comparison of explained variance (R2) per pRF size for visual regions of interest"
        )
    ...
    def compare_NSD_R2s(
        self, roi_masks, prf_dict, subject: str, slice: int, cmap: str = "afmhot"
    ):
        ...
        axes[0].set_title(
            "Haemodynamic Response Function signal fit R2 \nGLMdenoise Ridge Regression R-squared in percentages"
        )
        axes[0].axis("off")
        axes[1].imshow(
            R2_dict_onoff[subject]["full_R2"]["R2_ar"][:, :, slice], cmap=cmap
        )
        axes[1].set_title(
            "Voxel-wise variance explained for simple ON-OFF GLM model\nR-squared in percentages"
        )
        axes[1].axis("off")
        axes[2].imshow(
            self.nsp.utils.cap_values(
                prf_dict[subject]["nsd_dat"]["R2"]["prf_ar"], 0, 100
            )[:, :, slice],
            cmap=cmap,
        )
        axes[2].set_title(
            "Voxel-wise variance explained by pRF model\nR-squared in percentages"
        )
        axes[2].axis("off")
    ...
    def compare_masks(
        ...
        circle_dict = self.nsp.cortex.get_mask(
            dim=425,
            subject=subject,
            binary_masks=mask_dict,
        ...
        gaus = self.nsp.utils.make_gaussian_2d(
            425, circle_dict["x"], circle_dict["y"], circle_dict["pix_radius"]
        )
        full_gaus = self.nsp.utils.make_gaussian_2d(
            425, circle_dict["x"], circle_dict["y"], (circle_dict["size"] * (425 / 8.4))
        )
        cut_gaus = self.nsp.utils.css_gaussian_cut(
            425, circle_dict["x"], circle_dict["y"], (circle_dict["size"] * (425 / 8.4))
        )
        ...
        plot_mask(
            axs[2],
            full_gaus,
            "Full CSS input responsive Gaussian \n with prf_size as sigma",
        )
