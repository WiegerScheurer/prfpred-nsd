...
def get_codebase_home():
    config_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "rfpred_config.json"
    )
...
class Stimuli:
    ...
    def __init__(self, NSPobject):
        """
        Initialize the Stimuli class with a reference to the NSPobject.
        Args:
            NSPobject: Reference to the main class instance
        """
        self.nsp = NSPobject
        pass
    ...
    def show_stim(
        ...
        """
        Display or return a stimulus image from the NSD dataset.
        Args:
            img_no (str or int): Image number or 'random' for random selection.
            small (bool): If True, display smaller image, useful for quick notebook checkz
            hide (bool): If True, do not display image.
            crop (bool): If True, crop the image using provided parameters.
            ecc (int): Eccentricity for crop box location.
            angle (float): Angle for cropping.
            radius (float): Radius for cropping.
            custom_dir (str): Custom directory for stimuli.
        Returns:
            tuple: (test_image, image_no) where test_image is the image array and image_no is the index.
        """
        # Also useful as reference for accessing .hdf5 image files, these are all 73000 of them, as np.arrays
        ...
        stim_dir = (
            f"{self.nsp.nsd_datapath}/nsddata_stimuli/stimuli/nsd/"
            if not custom_dir
            else custom_dir
        )
        stim_files = [
            f for f in os.listdir(stim_dir) if os.path.isfile(os.path.join(stim_dir, f))
        ]
        ...
        with h5py.File(f"{stim_dir}{stim_files[0]}", "r") as file:
            ...
            if crop:
                ...
                bounds = self.nsp.utils.get_cropbox(
                    angle, ecc, radius, full_image.shape[0]
                )

                test_image = full_image[bounds[2] : bounds[3], bounds[0] : bounds[1]]
            else:
                test_image = img_brick_dataset[image_no]
        hor = ver = 10 # Not chique but functional and no risk here
        if small:
            hor = ver = 5
        if hide is not True:
    ...
    def mask_img(
        self, img_no: str | int = "random", radius: int = 1, small: bool = True
    ):
        """
    ...
    def calc_rms_contrast_lab(
        self,
        rgb_image: np.ndarray,
        mask_w_in: np.ndarray | None = None,
        rf_mask_in: np.ndarray | None = None,
        normalise: bool = True,
        plot: bool = False,
        cmap: str = "gist_gray",
        crop_post: bool = False,
        nsd_idx: int | None = None,
        lab_idx: int = 0,
        cropped_input: bool = True,
    ) -> float:
        """ "
        ...
        rf_mask_in = (
            np.ones((rgb_image.shape[:-1])) if rf_mask_in is None else rf_mask_in
        )
    ...
    def rms_single(
        ...
        mask_w_in = self.nsp.utils.css_gaussian_cut(dim, x, y, radius).reshape(
            (425, 425)
        )
        rf_mask_in = self.nsp.utils.make_circle_mask(
            dim, x, y, radius, fill="y", margin_width=0
        )
        ar_in = self.nsp.stimuli.show_stim(
            img_no=i, hide=bool(np.abs(plot_original - 1)), small=True
        )[0]
        ...
        return self.calc_rms_contrast_lab(
            ar_in,
            mask_w_in,
            rf_mask_in,
            normalise=normalise,
    ...
    def rms_all(
        ...
        """
        Compute RMS contrast for a batch of images in parallel.
        Args:
            start (int): Starting image index.
            n (int): Number of images.
            ecc_max (int): Maximum eccentricity.
            plot_original (bool): Whether to plot original images.
            plot_contrast (bool): Whether to plot contrast images.
            loc (str): Location ('center' or 'irrelevant_patch').
            crop_prior (bool): Crop before calculation.
            crop_post (bool): Crop after calculation.
            save_plot (bool): Save plots.
        Returns:
            pd.DataFrame: DataFrame of RMS values indexed by image number.
        """
        ...
        rms_dict = pd.DataFrame({"rms": rms_vec})

        rms_dict = rms_dict.set_index(np.array(img_vec))
        return rms_dict

    ################## WORKS, BUT NEED TO SET UP THE LGNPY PACKAGE AGAIN ###################
    lgnpy_set_up = False  # Check if this works
    ...
    def get_scce_contrast(
        ...
        """
        Compute Spatial Coherence and Contrast Energy features using LGNpy package.
        Args:
            rgb_image (np.ndarray): Input RGB image.
            plot (str): Whether to plot ('y' or 'n').
            cmap (str): Colormap for plots.
            crop_prior (bool): Crop before calculation.
            crop_post (bool): Crop after calculation.
            save_plot (bool): Save plots.
            return_imfovs (bool): Return imfov overlays.
            imfov_overlay (bool): Overlay imfov on plots.
            config_path (str): Path to LGN config file.
            lgn_instance (LGN): LGN instance.
            patch_center (tuple): Patch center coordinates.
            deg_per_pixel (float): Degrees per pixel.
        Returns:
            tuple: ce, sc, beta, gamma, imfovbeta, imfovgamma, edge_dict
        """
        ...
        if lgnpy_set_up:
            ...
            if imfov_overlay:
                _, beta_rad = self.nsp.utils._get_circle_outline(
                    imfovbeta,
                    deg_per_pixel=deg_per_pixel,
                    patch_center=(patch_x, patch_y),
                )
                _, gamma_rad = self.nsp.utils._get_circle_outline(
                    imfovgamma,
                    deg_per_pixel=deg_per_pixel,
                    patch_center=(patch_x, patch_y),
                )
                ...
                for i, img_name in enumerate(images):
                    ...
                    if imfov_overlay:
                        ...
                        circle = (
                            imfov_x,
                            imfov_y,
                            beta_rad if "par" in img_name else gamma_rad,
                        )
            ...
            print(
                "LGNpy package is not set up. Please set up the package to compute the Spatial Coherence and Contrast Energy features."
            )
            pass

    # idem dito; requires LGN to be setup
    def scce_single(
        self,
        args,
        ecc_max=2.8,
        ...
        Args:
            args (tuple): Arguments for computation.
            ecc_max (float): Maximum eccentricity.
            loc (str): Location ('center' or 'irrelevant_patch').
            plot (str): Whether to plot ('y' or 'n').
            cmap (str): Colormap for plots.
            return_imfovs (bool): Return imfov overlays.
            ...
            lgn_instance (LGN): LGN instance.
        Returns:
            tuple or None: Computed features or None if LGNpy not set up.
        """
        # Check if lgnpy is set up, otherwise pass
        if lgnpy_set_up:
    ...
    def features(
        ...
        if (
            pred
        ):  # TODO: adapt so that I also use this method for the central patches. # DONE
            ...
            feature_paths = [
                ...
                f"{self.nsp.own_datapath}/visfeats/pred_archcheck/all_predestims_vgg11-conv.csv",
                f"{self.nsp.own_datapath}/visfeats/pred_archcheck/all_predestims_vgg11-dense.csv",
            ]
        return {
            os.path.basename(file): self.nsp.datafetch.fetch_file(file)
    ...
    def get_rms(
        ...
        if subject is None:
            ...
            if peri_pars is None:
                ...
                df_list = [
                    pd.DataFrame(rms[subj][rms_loc_relevance]) for subj in rms.keys()
                ]
    ...
    def get_scce(
        ...
        indices = (
            self.imgs_designmx()[subject]
            if subject is not None
            else list(range(0, 73000))
        )  # Get the 73k-based indices for the specific subject
    ...
    def baseline_feats(self, subject: str, feat_type: str, outlier_bound: float = 0.3):
        """
        Input options:
        ...
        X = self.nsp.utils.replace_outliers(
            np.array(
                self.nsp.stimuli.features()[file_name]["subj01"][category][key]
            ).reshape(-1, 1),
            m=outlier_bound,
        )
        return zs(X)

    #### Slightly more efficient version of the above function, and also skips the ridiculous "features" function from prehistoric times.
    def unpredictability_feats(
        self,
        ydict: dict,
        subject: str = None,
        content: bool = True,
        ...
        """
        Efficiently creates feature matrix based on specified inclusion criteria.
        Maintains all original functionality with optimized code structure.
        """
        unpredfile = self.nsp.datafetch.fetch_file(custom_path)

        predfeatnames = [name for name in unpredfile.columns if name != "img_ices"]

        # Sort predfeatnames based on the integer in the filename
        predfeatnames = sorted(
            predfeatnames,
            key=lambda name: (
                int(re.findall(r"\d+", name)[-1]) if re.findall(r"\d+", name) else 0
            ),
        )

        if subject is not None:
            indices = self.nsp.stimuli.imgs_designmx()[subject]
        else:
            indices = np.ones((73000,)).astype(bool)
        ...
        data = {
            name: zs(
                self.nsp.utils.std_dev_cap(
                    unpredfile[name].fillna(0.00001), num_std_dev=cutoff_bound
                )
            )[indices]
            for name in predfeatnames
        }
    ...
    def plot_unet_feats(
        ...
        """
        with open(
            f"{self.nsp.own_datapath}/{subject}/pred/featmaps/{scale}/feats_gt_np_{batch}.pkl",
            "rb",
        ) as f:
        ...
        for i in range(num_feature_maps):
            ...
            axes[i].imshow(
                feats_gt_np[layer][random.randint(0, 500), i, :, :], cmap=cmap
            )
    ...
    def alex_featmaps_old(
        self,
        layers: list,
        pcs_per_layer: Union[int, str] = "all",
        subject: str = "subj01",
        ...
        for n_layer, layer in enumerate(layers):
            this_X = np.load(
                f"{self.nsp.own_datapath}/subj01/center_strict/alex_lay{layer}.npy"
            )
            if n_layer == 0:
                ...
                X_all = np.hstack((X_all, this_X[:, :cut_off]))
        ...
        if plot_corrmx:
            ...
            X_split = np.hsplit(
                X_all, len(layers)
            )  # Amazing function, splits the array into n arrays along the columns
            ...
            for i in range(len(layers)):
                for j in range(len(layers)):
                    corr_matrix[i, j] = np.corrcoef(
                        X_split[i].flatten(), X_split[j].flatten()
                    )[0, 1]
    ...
    def alex_featmaps(
        ...
        for n_layer, cnn_layer in enumerate(layers):
            if n_layer == 0:
                full_img_alex = np.load(
                    f"{self.nsp.own_datapath}/{subject}/encoding/{modeltype}/regprepped_featmaps{smallpatch_str}_layer{cnn_layer}.npy"
                )
            else:
                full_img_alex = np.hstack(
                    (
                        full_img_alex,
                        np.load(
                            f"{self.nsp.own_datapath}/{subject}/encoding/{modeltype}/regprepped_featmaps{smallpatch_str}_layer{cnn_layer}.npy"
    ...
    def imgs_designmx(self):
        ...
        Create design matrix containing ordered indices of stimulus presentation per subject.
        Returns:
            dict: Mapping of subject to stimulus indices.
        """

        subjects = os.listdir(f"{self.nsp.nsd_datapath}/nsddata/ppdata")
        exp_design = (
            f"{self.nsp.nsd_datapath}/nsddata/experiments/nsd/nsd_expdesign.mat"
        )
        ...
        img_index_seq = (
            mat_data["subjectim"] - 1
        )  # Change from matlab to python's 0-indexing
    ...
    def random_designmx(self, idx_min=0, idx_max=40, n_img=20):
        """
        Generate random design matrix for testing functions.
        Args:
            idx_min (int): Minimum index value.
            idx_max (int): Maximum index value.
            n_img (int): Number of images per subject.
        Returns:
            dict: Mapping of subject to random stimulus indices.
        """
    ...
    def unpred_corrmatrix(
        ...
        if cnn_type == "alexnet":
            ...
            predfeatnames = [
                name
                for name in list(self.features()[file_str].keys())
                if name.endswith(loss_calc) and name.startswith("content")
            ]
            ...
            predfeatnames = [
                name
                for name in self.features()[file_str].columns
                if name.endswith(loss_calc) and name.startswith("content")
            ]
        ...
        # Build dataframe
        data = {
            name: self.features()[file_str][name][indices] for name in predfeatnames
        }
    ...
    def plot_correlation_matrix(
        ...
        include_ce_l: bool = True,
        include_sc: bool = True,
        include_sc_l: bool = True,
        include_ce_new: bool = True,
        include_sc_new: bool = True,
        cmap: str = "copper_r",
        cnn_type: str = "alexnet",
        loss_calc: str = "MSE",
        peripheral: bool = False,
        ...
        if cnn_type == "alexnet":
            ...
            predfeatnames = [
                ...
                if name.endswith(loss_calc)
                and name.startswith("content")
                and name != "content_loss_0_MSE"
            ]
        ...
        ticks = [
            f'Pred {int(re.search(r"_([0-9]+)", name).group(1))+1}'
            for name in predfeatnames
        ]
        if include_rms:
            ticks.append("RMS 1°")
        ...
        if include_ce_new:
            ticks.append(
                "CE"
            )  # This used to be CE new, but for now, it's the same as the old one
        if include_sc_new:
            ticks.append("SC")
        ...
        plt.title(
            f"Correlation matrix for the MSE content loss values per\n{cnn_type} layer, and the baseline features"
        )
        plt.show()

    def cormx(
        self, input_matrix: np.ndarray, labels: list | None = None, cmap: str = "RdGy"
    ):
        ...
        plt.title(
            f"Correlation matrix for the MSE content loss values per layer, and the baseline features"
        )
        plt.show()
    ...
    def extract_features(
        ...
        class ImageDataset(Dataset):
            ...
            def __getitem__(self, idx):
                ...
                if img_crop:
                    imgnp = self.supclass.show_stim(
                        img_no=img_id, hide=True, small=True
                    )[0][163:263, 163:263]
                else:
                    imgnp = self.supclass.show_stim(
                        img_no=img_id, hide=True, small=True
            ...
            while True:  # Keep trying until successful
                try:
                    ...
                    for _, d in tqdm(enumerate(dataloader), total=len(dataloader)):
                        ...
                        ft_flat = torch.hstack(
                            [torch.flatten(l, start_dim=1) for l in ft.values()]
                        )
                        # Fit PCA to batch
    ...
    def plot_features(
        ...
        if random_cmap:
            print(
                f"The Lord has decided for you to peek into feature space through the lens of {this_cmap}"
            )
    ...
    ##### TODO: Give this a better name, and change a bit so it works for different subjects. It is not really random, but
    ...
    def rand_img_list(
        ...
        mask = (
            self.nsp.utils.make_circle_mask(dim, x, y, radius, fill="y", margin_width=0)
            == 0
        )
    ...
    def predplot(
        ...
        # If it is a list, set specific_imgs to that list
            ...
            dmx = (
                self.imgs_designmx()
            )  # A range of images based on the subject-specific design matrix
            subj_imgs = list(dmx[subject])
            specific_imgs = subj_imgs[start_img : start_img + n_imgs]
        else:
            raise TypeError(
                "Please select a valid image selection method: random, subject_based or a list of specific image indices"
            )

        # Get the images, masks and image numbers based on the specific image selection
        imgs, masks, img_nos = self.rand_img_list(
            n_imgs,
            asPIL=True,
            add_masks=True,
            mask_loc=mask_loc,
            ecc_max=ecc_max,
            select_ices=specific_imgs,
        )
        ...
        # Run them through the U-Net
        payload_full = unet.analyse_images(
            imgs, masks, return_recons=True, eval_mask=None
        )
        payload_crop = unet.analyse_images(
            imgs, masks, return_recons=True, eval_mask=eval_mask
        )
        ...
        for img_idx in range(len(imgs)):
            ...
            fig, axes = plt.subplots(
                ...
                7,
                figsize=(16, 8),
                gridspec_kw={"width_ratios": [2, 2, 2, 0.4, 2.5, 1.3, 2.5]},
            )  # Create 4 subplots
            for ax in axes[:, 3]:
                ax.axis("off")
            for ax in axes[:, 5]:
            ...
            for eval_size in range(2):
                ...
                for loss_type in ["content", "style"]:
                    ...
                    yrange = [0, 5] if loss_type == "content" else [0, 0.05]
                    ylogrange = [0.1, 100] if loss_type == "content" else [0.00001, 0.1]
                    n_layers = 5
                    losses = {}
                    MSE = []
                    L1 = []
                    ...
                    for i in range(n_layers):
                        MSE.append(
                            round(this_payload[f"{loss_type}_loss_{i}_MSE"][img_idx], 3)
                        )  # Get the loss for each layer
                        L1.append(
                            round(this_payload[f"{loss_type}_loss_{i}_L1"][img_idx], 3)
                        )  # Get the loss for each layer
                        losses["MSE"] = MSE
                        losses["L1"] = L1

                    # Plot the loss values
                    axes[eval_size, ntype].plot(
                        range(1, n_layers + 1),
                        L1,
                        marker="o",
                        color="crimson",
                    ...
                    if eval_size == 1:
                        axes[eval_size, ntype].set_xlabel(
                            "Feature space (Alexnet layer)"
                        )
                    ...
                    if loss_type == "content":
                        ax_mse.tick_params(
                            axis="y", labelcolor="cornflowerblue", labelsize=12
                        )

                    if log_y_MSE == "y" and loss_type == "content":
                        ax_mse.set_yscale(
                            "log"
                        )  # Set y-axis to logarithmic scale for MSE
                        ax_mse.set_ylabel(
                            "MSE Loss (log)", color="cornflowerblue", fontsize=14
                        )
                        ax_mse.set_ylim(ylogrange[0], ylogrange[1])
                        ax_mse.grid(False)
                        axes[eval_size, ntype].set_ylabel(
                            "L1 Loss (linear)", color="crimson", fontsize=14
                        )
                        axes[eval_size, ntype].set_ylim(
                            [yrange[0], yrange[1]]
                        )  # Set the range of the y-axis for L1
                    else:
                        axes[eval_size, ntype].set_ylabel(
                            "Loss value", color="white", fontsize=14
                        )
                        axes[eval_size, ntype].set_ylim([yrange[0], yrange[1]])

                    axes[eval_size, ntype].xaxis.set_major_locator(
                        plt.MaxNLocator(integer=True)
                    )  # Ensure x-axis ticks are integers
                    if loss_type == "content":
                        axes[eval_size, ntype].tick_params(
                            axis="y", labelcolor="crimson", labelsize=12
                        )
                ...
                fig.suptitle(
                    ...
                    f"Pixel loss MSE: {round((this_payload['pixel_loss_MSE'][img_idx]).astype('float'), 4)}",
                    fontsize=14,
                )
                ...
                for i, key in enumerate(["input_gt", "input_masked", "out_composite"]):
                    ...
                    axes[eval_size, i].axis(
                        "off"
                    )  # To keep images square and hide the axes
    ...
    def get_img_extremes(
        ...
        if top == "unpred":
            n_indices = sorted_indices[
                -final_img : -start_img if start_img != 0 else None
            ]
    ...
    def load_gabor_output(
        ...
        if peri_ecc is None and peri_angle is None:
            gabor_dir = (
                f"{self.nsp.own_datapath}/visfeats/gabor_pyramid/batches_{file_tag}"
            )
        ...
        for file_no, file in enumerate(files):
            ...
            if verbose:
                ...
                print(
                    f"Batch {file_no} shape: {this_batch.shape}"
