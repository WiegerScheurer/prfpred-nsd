...
# # # Remove the voxeldict object to save memory
# # del voxeldict
...
# # def rand_img_list(n_imgs, asPIL:bool = True, add_masks:bool = True, 
# #                   mask_loc: str|np.ndarray = 'center', ecc_max = 1, 
# #                   select_ices = None, in_3d:bool = False):
# #     imgs = []
# #     img_nos = []
...
# # unet=UNet(checkpoint_name='pconv_circ-places20k.pth',feature_model=this_dcnn)
# # print("Hier niet meer?")
# # imgs, masks, img_nos = rand_img_list(n_imgs, asPIL = True, add_masks = True, mask_loc = mask1, 
# #                                      ecc_max = 1, select_ices = select_ices, in_3d = False)
...
# # dir_path = NSP.own_datapath + f'/visfeats/pred_archcheck'
# # # dir_path = f'/home/rfpred/data/custom_files/visfeats/peripheral/ecc{args.eccentricity}_angle{args.angle}/pred'
# # os.makedirs(dir_path, exist_ok=True)

# # with h5py.File(f'{dir_path}/pred_payloads{args.startimg}_{args.endimg}_{this_dcnn}.h5', 'w') as hf:
# #     for key, value in payload_light.items():
# #         hf.create_dataset(key, data=value)
# #     print('Light payload saved succesfully')
...
#     description="Get the predictability estimates for a range of images of a subject"
# )
# predparser.add_argument("startimg", type=int, help="The first image of this batch")
# predparser.add_argument("endimg", type=int, help="The last image of this batch")
# args = predparser.parse_args()

# print(f"Processing images from {args.startimg} to {args.endimg}\n")
...
# min_nsd_R2 = 0
# min_prf_R2 = 0
# eval_fact = np.sqrt(1.2)  # Evaluation mask scaling factor
# mask_radius = 100
...
# def draw_circmask(dims, maskrad, offset=(0,0), invert=True):
#     """Create a circular mask with given dimensions and radius"""
#     y, x = np.ogrid[:dims[0], :dims[1]]
#     center_x, center_y = dims[1] // 2 + offset[0], dims[0] // 2 + offset[1]
#     mask = (x - center_x)**2 + (y - center_y)**2 <= maskrad**2
#     return ~mask if invert else mask

# def scale_square_mask(mask_in, scale_fact=np.sqrt(1.5), mask_val=1, min_size=50):
#     """Scale a square mask by a given factor while maintaining square proportions"""
...
# def rand_img_list(n_imgs, mask_loc, select_ices=None):
#     """Generate random images with corresponding masks"""
#     imgs, masks, img_nos = [], [], []
...
#         if i == 0:  # Only need to calculate mask dimensions once
#             dim = img.shape[0]
#             if isinstance(mask_loc, np.ndarray):
#                 bounds = get_bounding_box(mask_loc)
#                 patch_rad = bounds[1] - bounds[0]
#                 x = bounds[0] + patch_rad/2
#                 y = bounds[2] + patch_rad/2
...
#     mask = (make_circle_mask(dim, x, y, radius, fill='y', margin_width=0) == 0)
#     masks = [Image.fromarray(mask)] * n_imgs
...
# # Initialize Natural Spatial Predictor
# NSP = NatSpatPred()
# NSP.initialise(verbose=True)
# rl = Reloader()
...
# # Create central patch mask (using last ROI for mask generation)
# roi = rois[-1]
# voxel_sieve = VoxelSieve(
#     NSP,
#     prf_dict,
#     roi_masks,
#     subject="subj01",
...
# rf_mask_in = make_circle_mask(425, 212.5, 212.5, mask_radius, fill='y', margin_width=0)
# eval_mask = scale_square_mask(
#     ~np.array(masks[0]), 
#     min_size=80, 
#     scale_fact=eval_fact
# )
...
# total_time = end_time - start_time
# print(f'\nProcessing took {total_time} seconds ({total_time/60} minutes)')
# print(f"Average time per image: {total_time/n_imgs} seconds\n")
...
# # Save results
# dir_path = NSP.own_datapath + '/visfeats/pred_archcheck'
# os.makedirs(dir_path, exist_ok=True)
...
min_prf_R2 = 0
eval_fact = np.sqrt(1.2)  # Evaluation mask scaling factor
mask_radius = 100  # Radius for circular mask in pixels
default_eccentricity = 1  # Default eccentricity for mask placement
...
def rand_img_list(n_imgs, mask_loc, select_ices=None):
    ...
    for i in range(n_imgs):
        ...
        if i == 0:  # Only need to calculate mask dimensions once
            ...
            if isinstance(mask_loc, np.ndarray):
                # If mask_loc is an array, calculate position from its bounds
                bounds = get_bounding_box(mask_loc)
                patch_rad = (bounds[1] - bounds[0]) / 2
                x = bounds[0] + patch_rad
                y = bounds[2] + patch_rad
                radius = patch_rad
            else:
                # Default to central position with standard radius
                x = y = center
                radius = mask_radius
    ...
    # Create circular mask and invert it (True where mask should be applied)
    mask = (make_circle_mask(dim, x, y, radius, fill='y', margin_width=0) == 0)
    masks = [Image.fromarray(mask)] * n_imgs
...
# Create scaled evaluation mask (slightly larger than the original mask)
print("Creating evaluation mask...")
scaled_eval_mask = scale_square_mask(
    ~np.array(masks[0]),  # Invert the mask for evaluation
    min_size=80, 
    scale_fact=eval_fact
)
...
dir_path = NSP.own_datapath + '/visfeats/pred_archcheck'
os.makedirs(dir_path, exist_ok=True)
output_file = f'{dir_path}/pred_payloads{args.startimg}_{args.endimg}_{this_dcnn}.h5'
